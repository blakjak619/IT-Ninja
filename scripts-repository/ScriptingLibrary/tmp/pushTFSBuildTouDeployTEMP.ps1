...
[string]$packagePath = md $srcLocation"\temp"
 ...
 # Determine type of coponent, either APP or CFG, and the package location
    if($buildDefinition -match  "\.CI")
    {
        ...
        [string[]] $determinePackageFolder = Get-ChildItem ($binLocation + "\_PublishedWebsites") | ?{ $_.PSIsContainer } 
            ...
            if($determinePackageFolder -match "\.Services_Package")
            {
                [string]$packageFolder = ".Services_Package"                
            } 
            elseif($determinePackageFolder -match "\.Website_Package")
            {
                [string]$packageFolder = ".Website_Package"                
            }
            elseif($determinePackageFolder -match "_Package")
            {
 ...
 [string]$component = $appStack+"_"+$udAppType+"_"+$componentName 
