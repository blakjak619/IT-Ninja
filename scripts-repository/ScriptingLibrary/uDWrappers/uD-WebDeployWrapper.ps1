...
[String]$AppName,
[String]$wwwRoot,
$WCDropFolder = "\\10.13.0.206\uDeploy_repository\WebConfigExport"  )



$Usage = @"
#-------------------------------------------------------------------------
# Solution: uD-WebDeployWrapper.ps1
# Author: Todd Pluciennik
# Updated: 7/17/14
# Version: 2.3
<#
# Description:
- Executes webdeploy with the package and set parameters path passed.
...
# Usage: 
  ...
  Parameters:
        ...
        -AppName <uDeploy appName property>
...
# Version 1.01 - Leveraged ..\includes dir
# Version 1.5  - Passing directories vs. files, also added SetParameters.xml decision logic 
# Version 2.0  - XML collapse logic for SetParameters.xml
# Version 2.1  - Add NoClobber switch to LaunchProcessAndWait
# Version 2.2  - Added AppName and wwwRoot params to determine and display transformed web.config (nonprod only)
# Version 2.3  - Added logic to drop transformed (nonprod) web.config to a file share WCDropFolder (RITM0112131)
#-------------------------------------------------------------------------
...
$CLA = "-source:package='$Package' -dest:auto,computerName='localhost',includeAcls='False' -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:`"$SetParamFile`""
...
if ("$EnvName" -ne "PROD") { 
    ...
    if (Test-path $webconfigpath -PathType Leaf){
        ...
        # copy to drop folder, this only works for NON DMZ applications (i.e. will NOT work for SOAPRX)
        if ("$AppStack" -ne "SOAPRX" ) {
            if ($WCDropFolder) {
